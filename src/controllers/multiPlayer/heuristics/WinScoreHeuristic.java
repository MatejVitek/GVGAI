...
public class WinScoreHeuristic extends StateHeuristicMulti {
	...
	public double evaluateState(StateObservationMulti stateObs, int playerID) {
		...
		if (gameOver && (win == Types.WINNER.PLAYER_LOSES || oppWin == Types.WINNER.PLAYER_WINS)) return HUGE_NEGATIVE;

		if (gameOver && (win == Types.WINNER.PLAYER_WINS || oppWin == Types.WINNER.PLAYER_LOSES)) return HUGE_POSITIVE;
