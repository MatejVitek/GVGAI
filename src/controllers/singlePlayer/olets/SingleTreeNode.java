...
public class SingleTreeNode {
	...
	/**
	 * The expectimax value of a node (i.e. we back up the max value from its children nodes, plus the instant values observed when a simulation exited in this node, weighted proportionally to the
	 * number of exits
	 */
	...
	/**
	 * The bias given to a node according to the avatar location - a negative score bias is given if the location has been visited many times before
	 */
	...
	/**
	 * Updates nodes attributes in a tree; mostly used to reset number of simulations to 1 to reduce the weight of past simulations when salvaging a tree branch from one time step to the next
	 */
	...
	//
	// SingleTreeNode child = children[i];
	// double hvVal = child.expectimax + Agent.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + SingleMCTSPlayer.epsilon)) + SingleMCTSPlayer.randomGenerator.nextDouble() *
	// SingleMCTSPlayer.epsilon - this.tabooBias;
	//
	// // small sampleRandom numbers: break ties in unexpanded nodes
	// if (hvVal > bestValue) {
	// selected = child;
