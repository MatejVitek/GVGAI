...
/**
 * Created with IntelliJ IDEA. User: Diego Date: 17/10/13 Time: 12:13 This is a Java port from Tom Schaul's VGDL - https://github.com/schaul/py-vgdl
 */
public class Utils {
	...
	public static Polygon triPoints(Rectangle rect, Direction orientation) {
		Vector2d p1 = new Vector2d(rect.getCenterX() + orientation.x() * rect.getWidth() / 3.0, rect.getCenterY() + orientation.y() * rect.getHeight() / 3.0);
		Vector2d p2 = new Vector2d(rect.getCenterX() + orientation.x() * rect.getWidth() / 4.0, rect.getCenterY() + orientation.y() * rect.getHeight() / 4.0);
		Vector2d orthdir = new Vector2d(orientation.y(), -orientation.x());

		Vector2d p2a = new Vector2d(p2.x - orthdir.x * rect.getWidth() / 6.0, p2.y - orthdir.y * rect.getHeight() / 6.0);
		Vector2d p2b = new Vector2d(p2.x + orthdir.x * rect.getWidth() / 6.0, p2.y + orthdir.y * rect.getHeight() / 6.0);

		return new Polygon(new int[]{(int) p1.x, (int) p2a.x, (int) p2b.x}, new int[]{(int) p1.y, (int) p2a.y, (int) p2b.y}, 3);
	}
	...
	public static Direction processMovementActionKeys(boolean[] key_pressed, int idx) {
		...
		if (horizontal == 0) {
			...
			else if (horizontal == -1) return Types.DLEFT;
		}
		return Types.DNONE;
	}
