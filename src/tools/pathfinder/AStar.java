...
public class AStar {
	...
	public ArrayList<Node> getPath(Node start, Node goal) {
		...
		if (pathCache.containsKey(pathId)) return pathCache.get(pathId);
		return null;
	}
	...
	public ArrayList<Node> findPath(Node start, Node goal) {
		if (goal != null) {
			...
			if (path != null) pathCache.put(pathId, path);
	...
	private ArrayList<Node> _findPath(Node start, Node goal) {
		...
		while (openList.size() != 0) {
			...
			if (node.position.equals(goal.position)) return calculatePath(node);
		...
		if (!node.position.equals(goal.position)) return null;
