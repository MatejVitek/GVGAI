...
public class ShootAvatar extends OrientedAvatar {
	...
	private void shoot(Game game) {
		...
		VGDLSprite newOne = game.addSprite(itype, new Vector2d(this.rect.x + dir.x * this.lastrect.width, this.rect.y + dir.y * this.lastrect.height));

		if (newOne != null) {
			if (newOne.is_oriented) newOne.orientation = new Direction(dir.x, dir.y);
			reduceAmmo();
	...
	private boolean hasAmmo() {
		...
		if (resources.containsKey(ammoId)) return resources.get(ammoId) > 0;
	...
	public void postProcess() {
		...
		if (ammo != null) ammoId = VGDLRegistry.GetInstance().getRegisteredSpriteValue(ammo);
	}
