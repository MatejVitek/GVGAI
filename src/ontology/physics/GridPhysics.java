...
public class GridPhysics implements Physics {
	...
	public Types.MOVEMENT passiveMovement(VGDLSprite sprite) {
		...
		double speed;
		if (sprite.speed == -1) speed = 1;
		else speed = sprite.speed;

		if (speed != 0 && sprite.is_oriented) {
			if (sprite._updatePos(sprite.orientation, (int) (speed * this.gridsize.width))) return Types.MOVEMENT.MOVE;
		}
		return Types.MOVEMENT.STILL;
	}
	...
	public Types.MOVEMENT activeMovement(VGDLSprite sprite, Direction action, double speed) {
		...
		if (speed != 0 && action != null && !(action.equals(Types.DNONE))) {
			if (sprite.rotateInPlace) {
				...
				if (change) return Types.MOVEMENT.ROTATE;
			}

			if (sprite._updatePos(action, (int) (speed * this.gridsize.width))) return Types.MOVEMENT.MOVE;
		}
		return Types.MOVEMENT.STILL;
	}
