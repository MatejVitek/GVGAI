...
public class TimeEffect extends Effect implements Comparable<TimeEffect> {
	...
	/**
	 * timer for the effect, -1 by default. Indicates every how many steps this effects is triggered. Set in VGDL.
	 */
	public int timer = -1;

	/**
	 * Indicates the next time step when this effect will be automatically triggered without collisions. It is set by planExecution().
	 */
	...
	/**
	 * True if this is a time effect defined in VGDL using TIME. False if this is defined by an AddTimer effect
	 */
	public boolean isNative = true;

	/**
	 * The effect itself, that is triggered by this. it's a unary effect (the second sprite is always TIME).
	 */
	...
	public TimeEffect(InteractionContent ic, Effect delegate) {
		...
		if (nextExecution != -1) planExecution(null);
	}
	...
	public void execute(VGDLSprite sprite1, VGDLSprite sprite2, Game game) {
		...
		if (repeating) planExecution(game);
	}
