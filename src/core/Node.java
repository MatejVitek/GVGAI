...
/**
 * Created with IntelliJ IDEA. User: Diego Date: 04/10/13 Time: 16:34 This is a Java port from Tom Schaul's VGDL - https://github.com/schaul/py-vgdl
 */
public class Node {
	...
	public void insert(Node node) {
		if (this.indent < node.indent) {
			if (this.children.size() > 0) {
				if (this.children.get(0).indent != node.indent) throw new RuntimeException("children indentations must match");
			}
			...
			if (this.parent == null) throw new RuntimeException("Root node too indented?");
	...
	public String toString() {
		...
		if (this.children.size() == 0) return allStr;
