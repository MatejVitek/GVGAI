...
public class VGDLParser {
	...
	private void updateSet(String line) {
		...
		if (line.equalsIgnoreCase("InteractionSet")) currentSet = Types.VGDL_INTERACTION_SET;
		if (line.equalsIgnoreCase("LevelMapping")) currentSet = Types.VGDL_LEVEL_MAPPING;
		if (line.equalsIgnoreCase("TerminationSet")) currentSet = Types.VGDL_TERMINATION_SET;
	}
	...
	private void _parseSprites(ArrayList<Node> elements, String parentclass, HashMap<String, String> parentargs, ArrayList<String> parenttypes) {
		...
		for (Node el : elements) {
			...
			if (parentclass != null) sc.referenceClass = parentclass;
			...
			if (el.children.size() == 0) {
				if (VERBOSE_PARSER) System.out.println("Defining: " + identifier + " " + sc.referenceClass + " " + el.content.toString());
				...
				if (spriteClassName != null) parentclass = spriteClassName;
				...
				for (Node child : el.children) {
					...
					for (Integer subtype : childContent.subtypes) {
						if (!sc.subtypes.contains(subtype)) sc.subtypes.add(subtype);
					}
				}
			...
			if (ic.is_definition) // === contains ">"
			{
				...
				for (String obj2Str : ic.object2) {
					...
					if (obj1 != -1 && obj2 != -1) {
						...
						if (!game.getDefinedEffects().contains(newPair)) game.getDefinedEffects().add(newPair);
						...
						if (VERBOSE_PARSER) System.out.println("Defining interaction " + ic.object1 + "+" + obj2Str + " > " + ic.function);
