...
public class ForwardModel extends Game {
	...
	/**
	 * Private sampleRandom generator. Rolling the state forward from this state observation will use this sampleRandom generator, different from the one that is used in the real game.
	 */
	private Random randomObs;

	/**
	 * Boolean map of sprite types that are NPCs. npcList[spriteType]==true : spriteType is NPC.
	 */
	...
	/**
	 * Boolean map of sprite types that are immovable sprites. immList[spriteType]==true : spriteType is immovable sprite.
	 */
	private boolean immList[];

	/**
	 * Boolean map of sprite types that can move. movList[spriteType]==true : spriteType can move.
	 */
	private boolean movList[];

	/**
	 * Boolean map of sprite types that are resources. resList[spriteType]==true : spriteType is resource.
	 */
	private boolean resList[];

	/**
	 * Boolean map of sprite types that are portals or doors. portalList[spriteType]==true : spriteType is portal or door.
	 */
	private boolean portalList[];

	/**
	 * Boolean map of sprite types that created by the avatar. fromAvatar[spriteType]==true : spriteType is created by the avatar.
	 */
	private boolean fromAvatar[];

	/**
	 * Boolean map of sprite types that are unknown. unknownList[spriteType]==false : spriteType is unknown.
	 */
	private boolean unknownList[];

	/**
	 * Boolean map of sprite types that are not hidden. visibleList[spriteType]==true : sprite.hidden = false;
	 */
	...
	final public void update(Game a_gameState) {
		...
		for (int i = 0; i < spriteGroups.length; ++i) {
			...
			/**
			 * Index in the sprite group passed to the checkSpriteFeatures method to identify player in case of avatar sprites (index used as playerID in the avatars array).
			 */
	 ...
	 * 
	 * @param sprite
	 *            sprite to take the observation from.
	 */
	 ...
	 * @param moved
	 *            if it is a past observation, and it moved.
	 * @param oldPosition
	 *            the old position of this observation if it moved.
	 */
	 ...
	 * @param position
	 *            where the sprite was located last time seen.
	 */
	private void removeObservationFromGrid(Observation obs, Vector2d position) {
		...
		if (validX && validY) {
			...
			if (xPlus) observationGrid[x + 1][y].remove(obs);
			if (yPlus) observationGrid[x][y + 1].remove(obs);
			if (xPlus && yPlus) observationGrid[x + 1][y + 1].remove(obs);
		}
	}
	...
	private void addObservationToGrid(Observation obs, Vector2d position) {
		...
		if (validX && validY) {
			...
			if (xPlus) observationGrid[x + 1][y].add(obs);
			if (yPlus) observationGrid[x][y + 1].add(obs);
			if (xPlus && yPlus) observationGrid[x + 1][y + 1].add(obs);
		}
	}
	 ...
	 * 
	 * @param sprite
	 *            sprite to create the observation from.
	 ...
	 * 
	 * @param sprite
	 *            sprite to get/create the observation from.
	 * @return the observation object.
	 */
	...
	/**
	 * Initializes the variables of this game that have always a determined value at the beginning of any game.
	 */
	private void init() {
	 ...
	 * 
	 * @param action
	 *            Action to be performed by the avatar for this game tick.
	 */
	 ...
	 * 
	 * @param actions
	 *            array of actions of all players (index in array corresponds to playerID).
	 */
	...
	public ArrayList<Types.ACTIONS> getAvatarActions(int playerID, boolean includeNIL) {
		if (isEnded) return new ArrayList<Types.ACTIONS>();
		if (includeNIL) return avatars[playerID].actionsNIL;
	...
	public HashMap<Integer, Integer> getAvatarResources(int playerID) {
		...
		if (avatars[playerID] == null) return owned;
	 ...
	 * 
	 * @param groupArray
	 *            boolean map that indicates which sprite types must be considered.
	...
	private ArrayList<Observation>[] getPositionsFrom(boolean[] groupArray, Vector2d refPosition) {
		...
		for (int i = 0; i < groupArray.length; ++i) {
			...
			if (!unknownList[i] && spriteGroups[i].getFirstSprite() != null) checkSpriteFeatures(spriteGroups[i].getFirstSprite(), i);

			if (groupArray[i] && visibleList[i]) numDiffTypes++;
		}

		if (numDiffTypes == 0) return null; // Wait, no types? no sprites of this group then.

		ArrayList<Observation>[] observations = new ArrayList[numDiffTypes];
		Vector2d reference = refPosition;
		if (refPosition == null) reference = Types.NIL;
	 ...
	 * type Observation, ordered asc. by distance to the avatar. Each Observation holds the position, unique id and sprite id of that particular sprite.
	 *
	 * @param refPosition
