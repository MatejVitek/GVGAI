...
/**
 * Created with IntelliJ IDEA. User: Diego Date: 17/10/13 Time: 13:42 This is a Java port from Tom Schaul's VGDL - https://github.com/schaul/py-vgdl
 */
public abstract class Game {
	...
	/**
	 * Content objects for the different sprite types.. The index is the type of object Content encloses information about the class of the object and its parameters.
	 */
	protected Content[] classConst;

	/**
	 * List of template sprites, one for each object in the above "classConst" array.
	 */
	protected VGDLSprite[] templateSprites;

	/**
	 * Groups of sprites in the level. Each element of the array is a collection of sprites of a given type, which is also the index of the array.
	 */
	protected SpriteGroup[] spriteGroups;

	/**
	 * Relationships for collisions: double array of (list of) effects. Interaction between two sprites can trigger more than one effect. collisionEffects[] -> int id of the FIRST element taking part
	 * on the effects. collisionEffects[][] -> int id of the SECOND element taking part on the effects.
	 */
	...
	/**
	 * Historic of events related to the avatar happened during the game. The entries are ordered asc. by game step.
	 */
	...
	/**
	 * Flag that can only be set to true externally. If true, the agent is disqualified.
	 */
	...
	/**
	 * Avatars last actions. Array for all avatars in the game. Index in array corresponds to playerID.
	 */
	...
	public void initSprites(ArrayList<Integer> spOrder, ArrayList<Integer> sings, HashMap<Integer, SpriteContent> constructors) {
		...
		for (Map.Entry<Integer, SpriteContent> entry : entries) {
			...
			if (refClass != null && refClass.equals("Resource")) {
				VGDLSprite resourceTest = VGDLFactory.GetInstance().createSprite(entry.getValue(), new Vector2d(0, 0), new Dimension(1, 1));
		...
		if (!iSubTypes[wallId].contains(wallId)) iSubTypes[wallId].add(wallId);

		if (!iSubTypes[avatarId].contains(avatarId)) iSubTypes[avatarId].add(avatarId);
	...
	private int getSpriteCategory(VGDLSprite sp) {
		if (sp.is_avatar) return Types.TYPE_AVATAR;

		// Is it a resource?
		if (sp.is_resource) return Types.TYPE_RESOURCE;

		// Is it a portal?
		if (sp.portal) return Types.TYPE_PORTAL;

		// Is it npc?
		if (sp.is_npc) return Types.TYPE_NPC;

		// Is it immovable?
		if (sp.is_static) return Types.TYPE_STATIC;

		// is it created by the avatar?
		if (sp.is_from_avatar) return Types.TYPE_FROMAVATAR;
	...
	public VGDLSprite getTempAvatar(SpriteData sprite) {
		...
		if (((SpriteContent) classConst[avatarId]).referenceClass != null) {
			VGDLSprite result = VGDLFactory.GetInstance().createSprite((SpriteContent) classConst[avatarId], new Vector2d(), new Dimension(1, 1));
			if (result != null) {
				return result;
			}
		}
	...
	/**
	 * Return an array of termination data objects. These objects represents the termination conditions for the game
	 * 
	...
	protected void parseParameters(GameContent content) {
		...
		if (!this.getClass().equals(refClass)) {
			System.out.println("Error: Game subclass instance not the same as content.referenceClass" + " " + this.getClass() + " " + refClass);
			return;
		}
		...
		if (key_handler != null && key_handler.equalsIgnoreCase("Pulse")) CompetitionParameters.KEY_HANDLER = CompetitionParameters.KEY_PULSE;
	...
	protected void addSprite(VGDLSprite sprite, int itype) {
		...
		if (sprite.is_stochastic) this.is_stochastic = true;
	...
	public double[] playGame(Player[] players, int randomSeed, boolean isHuman, int humanID) {
		...
		if (isHuman && !wi.windowClosed && CompetitionParameters.killWindowOnEnd) {
			if (CompetitionParameters.dialogBoxOnStartAndEnd) {
				if (no_players == 1) {
					JOptionPane.showMessageDialog(frame, "GAMEOVER: YOU " + (avatars[humanID].getWinState() == Types.WINNER.PLAYER_WINS ? "WIN." : "LOSE."));
				}
				else {
					String sb = "";
					for (int i = 0; i < no_players; i++) {
	...
	private void setTitle(JEasyFrame frame) {
		...
		if (!isEnded) frame.setTitle(sb);
		else {
			for (int i = 0; i < no_players; i++) {
				if (avatars[i].getWinState() == Types.WINNER.PLAYER_WINS) sb += " [Player " + i + " WINS!]";
				else sb += " [Player " + i + " LOSES!]";
			}
		}
	...
	/**
	 * This is a standard game cycle in J-VGDL. It advances the game tick, updates the forward model and rolls an action in all entities, handling collisions and end game situations.
	 */
	...
	/**
	 * Handles the result for the game, considering disqualifications. Prints the result (score, time and winner) and returns the score of the game. Default player ID used 0 for single player games.
	 * 
	...
	public double[] handleResult() {
		...
		for (int i = 0; i < avatars.length; i++) {
			...
			else if (avatars[i].getWinState() == Types.WINNER.PLAYER_WINS) if (avatars[i].getScore() <= 0) avatars[i].setScore(1);
		}
	...
	/**
	 * Checks if the game must finish because of number of cycles played. This is a value stored in CompetitionParameters.MAX_TIMESTEPS. If the game is due to end, the winner is determined and the
	 * flag isEnded is set to true.
	 */
	protected void checkTimeOut() {
		if (gameTick >= CompetitionParameters.MAX_TIMESTEPS) {
			isEnded = true;
			for (int i = 0; i < no_players; i++) {
				if (avatars[i].getWinState() != Types.WINNER.PLAYER_WINS) avatars[i].setWinState(Types.WINNER.PLAYER_LOSES);
			}
		}
	}

	/**
	 * Prints the result of the game, indicating the winner, the score and the number of game ticks played, in this order.
	 */
	...
	/**
	 * Returns the complete result of the game (victory, score, timestep). Indicated in triplets, one per player.
	 * 
	...
	public void disqualify(int id) {
		...
		else avatars[id].disqualify(true);
		isEnded = true;
	}
	...
	/**
	 * Performs one tick for the game: calling update(this) in all sprites. It follows the opposite order of the drawing order (inverse spriteOrder[]). Avatar is always updated first. Doesn't update
	 * disabled sprites.
	 */
	protected void tick() {
		...
		for (Pair<Integer, Integer> p : definedEffects) {
			...
			for (Effect ef : collisionEffects[p.first][p.second]) {
				...
				if (shieldedEffects[p.first].size() > 0) {
					if (shieldedEffects[p.first].contains(new Pair(p.second, ef.hashCode))) continue;
				}
				...
				if (!noSprites[intId] && bucketList[intId].size() == 0) {
					...
					if (bucketList[intId].size() == 0) noSprites[intId] = true;
				}
				...
				if (first.size() == 0 || second.size() == 0) break;
									...
									s2loop: for (int idx2 = 0; idx2 < numSprites2; idx2++) {
										...
										if (!s2.is_disabled()) {
											if (s1 != s2 && s1.rect.intersects(s2.rect)) {
												...
												if (kill_list.contains(s1)) break s2loop; // Stop checking sprite 1 if it was killed.
	...
	private void executeEffect(Effect ef, VGDLSprite s1, VGDLSprite s2) {
		...
		if (s1 != null && s2 != null) addEvent(s1, s2);
	}

	private void addEvent(VGDLSprite s1, VGDLSprite s2) {
		if (s1.is_avatar) historicEvents.add(new Event(gameTick, false, s1.getType(), s2.getType(), s1.spriteID, s2.spriteID, s1.getPosition()));

		else if (s1.is_from_avatar) historicEvents.add(new Event(gameTick, true, s1.getType(), s2.getType(), s1.spriteID, s2.spriteID, s1.getPosition()));

		else if (s2.is_avatar) historicEvents.add(new Event(gameTick, false, s2.getType(), s1.getType(), s2.spriteID, s1.spriteID, s2.getPosition()));

		else if (s2.is_from_avatar) historicEvents.add(new Event(gameTick, true, s2.getType(), s1.getType(), s2.spriteID, s1.spriteID, s2.getPosition()));
	}
	...
	protected void clearAll(ForwardModel fm) {
		for (VGDLSprite sprite : kill_list) {
			...
			if (sprite.is_avatar)
				...
				for (int i = 0; i < no_players; i++)
				if (sprite == avatars[i]) avatars[i] = null;
	...
	public VGDLSprite addSprite(SpriteContent content, Vector2d position, int itype, boolean force) {
		...
		if (!anyother) {
			...
			{
				newSprite = VGDLFactory.GetInstance().createSprite(content, position, new Dimension(block_size, block_size));
	...
	/**
	 * Returns an array of type Types.WINNER containing the win state of all players in the game. Index in the array corresponds to playerID.
	 * 
