...
public class GVGExecutor {
	...
	public static void printHelp() {
		...
		// -g examples/gridphysics/aliens.txt -l examples/gridphysics/aliens_lvl0.txt:examples/gridphysics/aliens_lvl1.txt -ag controllers.singlePlayer.sampleMCTS.Agent -res output_game1.txt -sds 1342
		// -vis 1 -rep 2
		...
		// -g examples/gridphysics/aliens.txt:examples/gridphysics/camelRace.txt -l examples/gridphysics/aliens_lvl0.txt:examples/gridphysics/aliens_lvl1.txt;examples/gridphysics/camelRace_lvl0.txt
		// -ag controllers.singlePlayer.sampleMCTS.Agent -res o1.txt:o2.txt -sds 1342:111 -vis 1 -rep 2
		// -g examples/gridphysics/aliens.txt:examples/gridphysics/camelRace.txt -l examples/gridphysics/aliens_lvl0.txt:examples/gridphysics/aliens_lvl1.txt;examples/gridphysics/camelRace_lvl0.txt
		// -ag controllers.singlePlayer.sampleMCTS.Agent -res o1.txt:o2.txt -vis 0 -rep 2
	}
	...
	public static void main(String args[]) {
		...
		if (num_games != levelFiles.length) throw new RuntimeException("Number of games in -g and -l must match.");
		if (resultFiles != null && num_games != resultFiles.length) throw new RuntimeException("If result output files are provided, their number must match the number of games.");
		if (seeds != null && num_games != seeds.length) throw new RuntimeException("If seeds are provided, their number must match the number of games.");
		if (seeds == null) {
			...
			for (int i = 0; i < num_games; ++i) {
				...
				for (int j = 0; j < num_levels; ++j) {
					...
					for (int k = 0; k < repetitions; ++k) {
						...
						// Play!
						double[] result = ArcadeMachine.runOneGame(game, levels[j], visibility, agent, saveActions ? actionFile : null, seed, 0);
