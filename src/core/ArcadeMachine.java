...
public class ArcadeMachine {
	...
	public static double runOneGeneratedLevel(String gameFile, boolean visuals, String agentName, String actionFile, String levelFile, int randomSeed, boolean isHuman) {
		...
		/**
		 * playGame and runGame methods from the Game class take an array of players as argument, including all players in the game. As this method refers to single player games, an array is created
		 * containing only one element: the player created earlier. To get back just 1 score for the player, the first element in the score array is returned.
		 */
	...
	public static double[] replayGame(String game_file, String level_file, boolean visuals, String actionFile) {
		...
		try {
			...
			if (no_players < 2) {
				...
				System.out.println("Replaying game in " + game_file + ", " + level_file + " with seed " + seed + " expecting player to win = " + (win[0] == 1) + "; score: " + loggedScore
						+ "; timesteps: " + timesteps);
				...
				// display information
				System.out.println("Replaying game in " + game_file + ", " + level_file + " with seed " + seed + " expecting players' win states = " + thirdLine + "; scores: " + secondLine
						+ "; timesteps: " + timesteps);
	...
	public static void runGames(String game_file, String[] level_files, int level_times, String agentName, String[] actionFiles) {
		...
		for (String level_file : level_files) {
			for (int i = 0; i < level_times; ++i) {
				if (VERBOSE) System.out.println(" ** Playing game " + game_file + ", level " + level_file + " (" + (i + 1) + "/" + level_times + ") **");
	...
	public static void playGeneratedLevels(String gameFile, String[] actionFile, String[] levelFile, boolean isHuman) {
		...
		for (String file : levelFile) {
			...
			if (player == null) {
				...
				/**
				 * playGame method from Game class takes an array of players as argument, including all players in the game. As this method refers to single player games, an array is created
				 * containing only one element: the player created earlier. To get back just 1 score for the player, the first element in the score array is returned.
				 */
	...
	private static AbstractPlayer createPlayer(String playerName, String actionFile, StateObservation so, int randomSeed, boolean isHuman) {
		...
		try {
			...
			if (player != null) player.setup(actionFile, randomSeed, isHuman);
	...
	protected static Player createController(String playerName, int playerID, StateObservation so) throws RuntimeException {
		...
		try {
			...
			if (ect.exceededMaxTime()) {
				...
				if (VERBOSE) System.out.println("Controller initialization time: " + timeTaken + " ms.");
			}
	...
	private static boolean tearPlayerDown(Game toPlay, Player[] players, String actionFile, int randomSeed, boolean record) {
		...
		for (Player p : players) {
			...
			if (VERBOSE) System.out.println("Controller tear down time: " + timeTaken + " ms.");
			return true;
		}
	...
	private static final boolean isHuman(String agentName) {
		if (agentName.equalsIgnoreCase("controllers.multiPlayer.human.Agent") || agentName.equalsIgnoreCase("controllers.singlePlayer.human.Agent")) return true;
		return false;
	}
