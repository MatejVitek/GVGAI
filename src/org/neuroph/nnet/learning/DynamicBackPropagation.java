...
public class DynamicBackPropagation extends MomentumBackpropagation {
	...
	/*
	 * If network error of current epoch is higher than the network error of the previous epoch the learning rate is adjusted by minus 1 per cent of current learning rate. Otherwise the learning rate
	 * is adjusted by plus 1 per cent of current learning rate. So, learning rate increases faster than decreasing does. But if learning rate reaches 0.9 it switches back to 0.5 to avoid endless
	 * training. The lowest learning rate is 0.5 also to avoid endless training.
	 */
	protected void adjustLearningRate() {
		...
		/*
		 * if (this.totalNetworkError >= this.totalNetworkErrorInPreviousEpoch) { this.learningRate = this.learningRate * this.learningRateChange; if (this.learningRate < this.minLearningRate)
		 * this.learningRate = this.minLearningRate; } else { this.learningRate = this.learningRate * (1 + (1 - this.learningRateChange)); // *1.01 if (this.learningRate > this.maxLearningRate)
	...
	protected void adjustMomentum() {
		...
		if (this.momentum < this.minMomentum) this.momentum = this.minMomentum;
