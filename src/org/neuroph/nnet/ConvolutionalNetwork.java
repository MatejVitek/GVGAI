/**
 * Copyright 2013 Neuroph Project http://neuroph.sourceforge.net Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the License. You
...
public class ConvolutionalNetwork extends NeuralNetwork<ConvolutionalBackpropagation> {
	...
	public void setInput(double... inputVector) throws VectorSizeMismatchException {
		...
		for (int i = 0; i < inputLayer.getNumberOfMaps(); i++) {
			...
			for (Neuron neuron : map.getNeurons()) {
				if (!(neuron instanceof BiasNeuron)) neuron.setInput(inputVector[currentNeuron++]);
			}
		}
	}
	...
	public static class Builder {
		...
		private FeatureMapsLayer getLastFeatureMapLayer() {
			...
			if (layer instanceof FeatureMapsLayer) return (FeatureMapsLayer) layer;
