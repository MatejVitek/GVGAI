...
public class LevelGenerator extends AbstractLevelGenerator {
	...
	/**
	 * tradeoff between intended percentage of objects and the generated one from the number of created objects
	 */
	...
	private LevelCoverData getPercentagesCovered(GameDescription game) {
		...
		data.levelPercentage = (coverTradeOffPercentage + (1 - coverTradeOffPercentage) / (numberOfCreationalObjects + 1)) * ((collectableValue + 1) / (harmfulValue + collectableValue + 1))
				* maxCoverPercentage;
			...
			while (solidNumber > 0) {
				...
				while (length > 0) {
					...
					for (i = 0; i < directions.size(); i++) {
						...
						if (level.get(newPoint.x, newPoint.y) == null && placeSolid(level, newPoint.x, newPoint.y, randomSolid)) {
